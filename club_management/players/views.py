...
class refreeListView(ListView):
    model = refree
    template_name = 'refree_list.html'

    def get_context_data(self, **kwargs):
        context = super(refreeListView, self).get_context_data(**kwargs)
        q = refree.objects.all()
        context['object_list'] = q
        return context

class refreeDetailView(DetailView):
    model = refree
    template_name = 'refree_detail.html'


class stadiumListView(ListView):
    model = stadium
    template_name = 'stadium_list.html'

    def get_context_data(self, **kwargs):
        context = super(stadiumListView, self).get_context_data(**kwargs)
        q = stadium.objects.all()
        context['object_list'] = q
        return context

class stadiumDetailView(DetailView):
    model = stadium
    template_name = 'stadium_detail.html'